...
# UPDATED**  "Log2(FC)" = log2(mean(Treatment)/ mean(Control))
...
def log2fc_calculator(df):
    ...
    result_df['T_Mean'] = result_df[['T1', 'T2', 'T3', 'T4']].mean(axis=1)
    # Insert the gene_ID's to the result_df
    ...
    # Calculate the fold change and assign it to a new column
    result_df['Fold_Change'] = result_df['T_Mean'] / result_df['C_Mean']
    # Calculate the log2 fold change and assign it to a new column
    result_df['Log2_FC'] = np.log2(result_df['Fold_Change'])
